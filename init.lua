...
do
  ...
  vim.keymap.set('n', '<leader>cp', function() vim.fn.setreg('+', vim.fn.expand '%') end, { desc = 'Copy relative path to clipboard' })
...
do
  ...
  vim.api.nvim_create_autocmd('PackChanged', {
    callback = function(ev)
      ...
      if name == 'mcphub.nvim' and vim.fn.executable 'npm' == 1 then run_build(name, { 'npm', 'install', '-g', 'mcp-hub@latest' }, ev.data.path) end
    end,
  })
end
...
do
  ...
  require('conform').setup {
    ...
    format_on_save = function(bufnr)
      ...
      local ignored_filetypes = { 'sql', 'java', 'ruby', 'javascript', 'javascriptreact', 'c', 'cpp' }
  ...
  local function diff_format()
    if vim.bo.filetype == 'lua' then
      vim.notify 'Range formatting for Lua is not working properly.'
      return
    end
    ...
    local function format_range()
      ...
      local hunk
      while next(hunks) ~= nil and (not hunk or hunk.type == 'delete') do
      ...
      if hunk and hunk.type ~= 'delete' then
        ...
        local range = { start = { first, 0 }, ['end'] = { last - 1, #last_line } }
        require('conform').format({ range = range, async = true, lsp_format = 'fallback' }, function() vim.defer_fn(format_range, 1) end)
      end
    end
...
do
  ...
  local parsers = { 'bash', 'c', 'css', 'diff', 'html', 'lua', 'luadoc', 'markdown', 'markdown_inline', 'query', 'ruby', 'vim', 'vimdoc' }
